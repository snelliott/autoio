...
def test_ped_names():
    ...
    assert pedspecies == (('C3H8+H', 'CH3CH2CH2+H2'), ('C3H8+H', 'CH3CHCH3+H2'))
    assert pedoutput == 'thf_pyro_ped.out'
...
def test_ped_get_ped():
    ...
    assert np.isclose(
        ped_dct1[(('CH2O+H',), ('HCO+H2',), (None,))][1][500].index[1], 16.8339,
        atol=1e-3, rtol=1e-3)
    ...
    assert np.isclose(
        ped_dct2[(('C3H8+H',), ('CH3CH2CH2+H2',), (None,))][1][500].index[1], 3.685142,
        atol=1e-3, rtol=1e-3)
    assert np.isclose(
        ped_dct2[(('C3H8+H',), ('CH3CHCH3+H2',), (None,))][1][500].index[1], 6.735142,
        atol=1e-3, rtol=1e-3)
